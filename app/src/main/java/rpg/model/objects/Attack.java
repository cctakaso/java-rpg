...
import rpg.model.types.AttackType;
...
public class Attack extends Base{
  ...
  protected Conditions conditions;  // 攻撃に関連するステータス
  ...
  public Attack() {
    super();
    this.conditions = new Conditions(); // ステータスを空のStatusesオブジェクトで初期化
  }

  /**
   * 攻撃の種類とステータスを指定して初期化します。
   * @param type 攻撃の種類
   * @param conditions 攻撃に関連するステータス
   */
  ...
  public int usedMagicPoint() {
    return getTypePoint(ConditionType.MagicPoint);
  }
  ...
  public int getAtackedHealthPoint() {
    return getTypePoint(ConditionType.HealthPoint);
  }
  ...
  public int calcHealthDamage(Character attacker, Character reciver) {
    ...
    if (isHeal()) {
      ...
      attackerPoint= attacker.charStatus.getRatePoint(ConditionType.MagicAttack);
      reciverPoint = 1;
    }else if (isPhysicalOffence()) {
      atackHp = getAtackedHealthPoint();
      attackerPoint = attacker.charStatus.getRatePoint(ConditionType.Attack);
      reciverPoint = reciver.charStatus.getRatePoint(ConditionType.Defence);
    }else if (isMagicalOffence()) {
      atackHp = getAtackedHealthPoint();
      attackerPoint = attacker.charStatus.getRatePoint(ConditionType.MagicAttack);
      reciverPoint = reciver.charStatus.getRatePoint(ConditionType.MagicDefence);
    }else{
      atackHp = 0;
      attackerPoint = 1;
      reciverPoint = 1;
  ...
  public boolean isDefence() {
    return isPhysicalDefence() || isMagicDefence();
  }

  /**
   * 攻撃の種類がパーティーに影響を与えるかどうかを判定します。
   * <p>
   * 攻撃の種類がパーティー攻撃またはパーティー回復の場合、パーティーに影響を与えるとみなします。
   * </p>
   * @return パーティーに影響を与える攻撃かどうか
   */
  public boolean isAffectParty() {
  ...
  /**
   * 攻撃の種類が物理防御かどうかを判定します。
   * <p>
   * 攻撃の種類が防御の場合、物理防御とみなします。
   * </p>
   * @return 物理防御かどうか
   */
  public boolean isPhysicalDefence() {
    return this.type == AttackType.Protect;
  }
   ...
   * @return ステータスのポイント
   */
  private int getTypePoint(ConditionType type) {
